...
def DiffeoOUT(
    ...
    energy = enr_OT(
        source, target, match_coeff, path_coeff, inner_prod, div, vols, delta
    )
...
def DiffeoOT(
    ...
    def funopt(vecs):
        vecs = torch.from_numpy(vecs.reshape(T - 1, m, d)).to(
    ...
    vecs = torch.from_numpy(xopt.reshape(T - 1, m, d)).to(
        dtype=torchdtype, device=torchdeviceId
    )
