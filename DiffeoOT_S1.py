...
def DiffeoOT(mu_1,mu_2,grid,T,match_coeff,path_coeff,vecs,inner_prod,max_iter=10000):
    ...
    def funopt(vecs):
        vecs=torch.from_numpy(vecs.reshape(T-1,m)).to(dtype=torchdtype, device=torchdeviceId)
    ...
    vecs = torch.cat([torch.zeros((1,m)),torch.from_numpy(xopt.reshape(T-1,m))], dim=0).to(dtype=torchdtype, device=torchdeviceId)
    return vecs
