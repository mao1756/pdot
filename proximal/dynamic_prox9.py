...
def prox_J(m, f, gamma):
    a,b,c,d = 1, (2*gamma - f), gamma**2-2*gamma*f, -f*gamma**2 - gamma/2 * np.linalg.norm(m)**2
...
#        A[k,(P+1)*N+N+k]=1
    ...
    for k in range(P):
        A[N*(P+1)+2*N+k, N*(P+1)+(k+1)*N:N*(P+1)+(k+1)*N+N]=1*np.cos(2*np.pi*xc)
        A[N*(P+1)+2*N+P+k, N*(P+1)+(k+1)*N:N*(P+1)+(k+1)*N+N]=1*np.sin(2*np.pi*xc)
...
def measure_to_curve(mu):
    ...
    for i in range(n):
        ...
        S = S + mu_sorted[i,1]*np.array([np.cos(mu_sorted[i,0]), np.sin(mu_sorted[i,0])])
...
#    BtBinvsvd= np.dot(Vhb.T,np.dot(Sinv,Uhb.T))
    ...
    Atilde = np.zeros((N*(P+1)+2*N+2*P+1, N*(P+1)+N*(P+2)+N*(P+1)))
    Atilde[:-1,:] = A
    ...
    for l in range(L):
        I = mid_int(Gamma[0], Gamma[1])
        I=(I[0],I[1],Gamma[2])
        V = prox_jconj_unbalanced((V[0]+sigma*I[0], V[1]+sigma*I[1], V[2]+sigma*I[2]), sigma)

        Uprec = (U[0].copy(), U[1].copy(), U[2].copy())
        Iconj = mid_intconj(V[0], V[1])
        U = projC_unbalanced(Uprec[0]-tau*Iconj[0], Uprec[1] - tau*Iconj[1], Uprec[2] - tau*V[2], C, Ay, tau)
        j.append((fun_j_unbalanced(mid_int(U[0],U[1]),U[2])))
        Gamma = (U[0] + theta*(U[0]-Uprec[0]), U[1] + theta*(U[1]-Uprec[1]),U[2] + theta*(U[2]-Uprec[2]))
...
def prox_jconj_unbalanced(V, sigma):
    ...
    for k in range(n):
        for l in range(p):
            Mprox[k,l], Fprox[k,l], Zetaprox[k,l] = prox_J_unbalanced(V[0][k,l]/sigma, V[1][k,l]/sigma, V[2][k,l]/sigma, 1/sigma)
    
    #U = prox_j((V[0]/gamma, V[1]/gamma), 1/gamma)
    return (V[0]-sigma*Mprox, V[1]-sigma*Fprox, V[2]-sigma*Zetaprox)

def prox_J_unbalanced(m, f, zeta, gamma):
    a,b,c,d = 1, (2*gamma - f), gamma**2-2*gamma*f, -f*gamma**2 - gamma/2 * (m**2+zeta**2)
    f_et = pol_deg3.maxSol(a,b,c,d)
    if (f_et<=0):
        return (0,0,0)
    else:
