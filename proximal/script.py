...
V2 = np.array([[0.,0.],[1,1], [-2,4]])
E2 = np.array([[0,1],[1,2],[2,0]])
V2, E2 = geod.normalize_curve(V2, E2) # normalise les longueurs pour bien avoir un périmètre égal à 1
...
savemat('matching_constrained_Wasserstein_bis.mat',{'f0':f0,'f1':f1,'rho_path':rho,'m_path':m,'Ener':j})
...
f1=np.zeros((N,))
f1[0:(np.int(N/6))]= 1.
f1[(np.int(N/3)):(np.int(N/2))] = 1.
f1[(np.int(4*N/6)):(np.int(5*N/6))] = 1.
r=np.sum(f1)
f1=N*f1/r
...
savemat('matching_Wasserstein_Reuleaux3.mat',{'f0':f0,'f1':f1,'rho_path':rho,'m_path':m,'Ener':j})
...
savemat('matching_Mao_Wasserstein3_N_40_P_50.mat',{'f0':f0,'f1':f1,'rho_path':rho,'m_path':m,'Ener':j})
