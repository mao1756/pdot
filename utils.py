...
def gaussian(a,b,G,grid):
    ...
    mu=mu/mu.sum()
    return mu.reshape(m,m).to(dtype=torchdtype, device=torchdeviceId)

def resample_density(p,grid,X,T):
    X=X/T
    m=grid.shape[1]
    pm=p
...
def plot_pair(mu_1,mu_2):
    ...
    axes[0].matshow(mu_1.cpu().numpy(), vmin=0, vmax=vmax)
    axes[1].matshow(mu_2.cpu().numpy(), vmin=0, vmax=vmax)
    plt.show()

def plot_sequence(mu_1,mu_2,grid,ps,vecs,funs=None):
    vmax= 1.2*max(mu_1.max(),mu_2.max()).item()
    T=vecs.shape[0]
    fig, axes = plt.subplots(ncols=2*T+1, figsize=(20*T+10, 10))
    axes[0].pcolormesh(grid[0,:,:,0].cpu(),grid[0,:,:,1].cpu(), ps[0].transpose(0,1).cpu(), vmin=0, vmax=vmax)
    axes[0].set_xlim([-1, 1])
    axes[0].set_ylim([-1, 1])
    for i in range(0,T):
        X=vecs[i]
        axes[2*i+1].quiver(grid[0,:,:,0].cpu(),grid[0,:,:,1].cpu(),-(1/T)*X[:,:,0].cpu(),-(1/T)*X[:,:,1].cpu())
        axes[2*i+1].set_xlim([-1, 1])
        axes[2*i+1].set_ylim([-1, 1])
        axes[2*i+2].pcolormesh(grid[0,:,:,0].cpu(),grid[0,:,:,1].cpu(), ps[i+1].transpose(0,1).cpu(), vmin=0, vmax=vmax)
        axes[2*i+2].set_xlim([-1, 1])
        axes[2*i+2].set_ylim([-1, 1])
    plt.show()

def save_gif(mu_1,mu_2,ps, filename="results/array.gif"):
    vmax= 1.2*max(mu_1.max(),mu_2.max()).item()
